...
def test_sync_zoho_non_activity_post_is_flagged(tmp_path: Path) -> None:
    path = _write_pkg(
        tmp_path,
        "sync/zoho_crm.py",
        "import requests\n\n"
...
def test_sync_zoho_get_only_passes(tmp_path: Path) -> None:
    path = _write_pkg(
        tmp_path,
        "sync/zoho_crm.py",
        "import requests\n\ndef do():\n    return requests.get('/crm/v3/Contacts')\n",
    )
    assert scan_file(path) == []
